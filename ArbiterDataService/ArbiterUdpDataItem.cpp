...
ArbiterRC UdpDataItem::Update()
{
    ...
    {
        ArbiterTracer::WriteLine(ArbiterTracer::CATEGORY_SOCKET, ArbiterTracer::ACTION_SEND_TO, "Error When Sending: [" + m_sendStr + "]");
        ArbiterTracer::Error(ArbiterTracer::CATEGORY_SOCKET, ArbiterTracer::ACTION_SEND_TO, errno);
        rc = 1000 * ArbiterTracer::ACTION_SEND_TO + errno;
        goto Exit;
    }
    ArbiterTracer::WriteLine(ArbiterTracer::CATEGORY_SOCKET, ArbiterTracer::SERVERITY_INFO, "Message: [" + sendStr + "] sent successfully!");
